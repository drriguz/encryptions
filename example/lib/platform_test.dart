...
final List<PlatformTest> tests = [
  ...
  PlatformTest("AES/128/CBC/PKCS5Padding", () async {
    ...
    Uint8List encrypted = await aes.encrypt(plain);
    Uint8List decrypted = await aes.decrypt(encrypted);
    return (hex.encode(encrypted) == "64fb88a3f1a4d75d05c5508b2f2d48936167b6d75caf4867cca5db8dcf97af04") &&
        (hex.encode(decrypted) == hex.encode(plain));
  }),
  PlatformTest("AES/128/CBC/PKCS5Padding/1", () async {
    ...
    Uint8List decrypted = await aes.decrypt(encrypted);
    return hex.encode(encrypted) == "64fb88a3f1a4d75d05c5508b2f2d48933740cf89850b682d8082ca17223a8f0f" &&
  ...
  PlatformTest("AES/256/CBC/PKCS5Padding", () async {
    ...
    Uint8List encrypted = await aes.encrypt(plain);
    Uint8List decrypted = await aes.decrypt(encrypted);
    return (hex.encode(encrypted) == "514628247c36f3a964ed4f2e44b3cc81c30092415f2c38b0e2bfb0c9d7fb46d7") &&
        (hex.encode(decrypted) == hex.encode(plain));
  }),
  PlatformTest("AES/256/CBC/PKCS5Padding/1", () async {
    ...
    Uint8List encrypted = await aes.encrypt(plain1);
    Uint8List decrypted = await aes.decrypt(encrypted);
    return (hex.encode(encrypted) == "514628247c36f3a964ed4f2e44b3cc815de5343cfac5450b11e5de1522cefe7e") &&
        (hex.encode(decrypted) == hex.encode(plain1));
  }),
  ...
  PlatformTest("AES/128/ECB/PKCS5Padding", () async {
    ...
    Uint8List encrypted = await aes.encrypt(plain);
    Uint8List decrypted = await aes.decrypt(encrypted);
    return (hex.encode(encrypted) == "0e86eca7a8f24e2e65732eb01fe04fb27acdfbde18273836de549e9d028f6a24") &&
        (hex.encode(decrypted) == hex.encode(plain));
  }),
  PlatformTest("AES/128/ECB/PKCS5Padding/1", () async {
    ...
    Uint8List encrypted = await aes.encrypt(plain1);
    Uint8List decrypted = await aes.decrypt(encrypted);
    return (hex.encode(encrypted) == "0e86eca7a8f24e2e65732eb01fe04fb2c41c508ff2ac46c6c9224e1f9e6a0b8f") &&
        (hex.encode(decrypted) == hex.encode(plain1));
  }),
  ...
  PlatformTest("AES/256/ECB/PKCS5Padding", () async {
    ...
    Uint8List encrypted = await aes.encrypt(plain);
    Uint8List decrypted = await aes.decrypt(encrypted);
    return (hex.encode(encrypted) == "b6665cd69f001766eb2e852b4b6d1ae0adb4c9f63ac1c3cce511cb1edc9304e8") &&
        (hex.encode(decrypted) == hex.encode(plain));
  }),
  PlatformTest("AES/256/ECB/PKCS5Padding/1", () async {
    ...
    Uint8List encrypted = await aes.encrypt(plain1);
    Uint8List decrypted = await aes.decrypt(encrypted);
    return (hex.encode(encrypted) == "b6665cd69f001766eb2e852b4b6d1ae088fc68b1d65cfb0d70171925d1a818e2") &&
        (hex.encode(decrypted) == hex.encode(plain1));
  }),
  ...
  PlatformTest("argon2i", () async {
    ...
    String hash = hex.encode(await argon2.argon2i(password, salt));
    String hash1 = hex.encode(await argon2.argon2iIsolated(password, salt));
    return hash == hash1 && hash == "1fe7d8ae9f8946a5170aa6b96e8eea69b5f9351cbe457c4776e01f0b72a1e40e";
  }),
  PlatformTest("argon2d", () async {
    Argon2 argon2 = Argon2();
    String hash = hex.encode(await argon2.argon2d(password, salt));
    String hash1 = hex.encode(await argon2.argon2dIsolated(password, salt));
    return hash == hash1 && hash == "251c68a5591a838647b5afa7d1379cc63690daffcedb1e725528789014064fab";
  }),
  PlatformTest("argon2id", () async {
    Argon2 argon2 = Argon2();
    String hash = hex.encode(await argon2.argon2id(password, salt));
    String hash1 = hex.encode(await argon2.argon2idIsolated(password, salt));
    return hash == hash1 && hash == "f4d4a3ae373989af91e2e396e28762fe05417bd1d5d9e44d3afdc0e71497c902";
  }),
